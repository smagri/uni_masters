...
int main(int argc, char *argv[]){
  ...
  //ans: -1, -1
    ...
    // Using a pointer to the base class Shape to access the derived
    // class member functions.
    //
    // Can't use the one shapePtr for accessing different objects,
    // each object needs to have it's own pointer to the base class.
    Shape *shapePtrRec = &rectangle;
    fprintf(stderr, "\nset rectangle: width=%d, height=%d:\n", side, side);
    fprintf(stderr, "with shapePtrRec access:\n");
    fprintf(stderr, "area of rectangle=%d\n", shapePtrRec->getArea());
    fprintf(stderr, "perimiter of rectangle=%d\n", shapePtrRec->getPerimeter());

    Shape *shapePtrTri = &triangle;
    fprintf(stderr, "\nset isoseles triangle: base=%d, height=%d, side=%d:\n",
	    width, height, side);
    fprintf(stderr, "with shapePtrTri access:\n");
    fprintf(stderr, "area of isosceles triangle=%d\n", shapePtrTri->getArea());
    fprintf(stderr, "perimiter of isosceles triangle=%d\n",
	    shapePtrTri->getPerimeter());
