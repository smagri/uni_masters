...
public:
  ...
  void  detectCellTypes(double closest_point, double angle,
    ...
    //
    // Here we update  the ogMap image with new UK,  FS, O's depending
    // on the raidial vector's magnitude.
    //
    ...
    // LATER:
    //
    // True   ogMap   centre  is   at   cell   (image.rows/2, image.cols/s).
    //
...
//    double msg->angle_min = (angleDeg*(pi/180)); // start angle in radians
    ...
    //   laserBuffer.laserDeq.pop_front(closest_point);
    //   laserBuffer.laserAngleDeq.pop_front(angle);
    //   laserBuffer.laserRminDeq.pop_front(range_min);
    //   laserBuffer.laserRmaxDeq.pop_front(range_max);
    //   laserBuffer.timeStampDeq.pop_front(msg->header.stamp);
    // }
  ...
  void seperateThread() {
	...
	if(!image.empty()){
	  ...
	  // Update the current ogMap with new FT, FS, UK, O cell values.
	...
	} // end: if(!image.empty()) LATER check???
	std::this_thread::sleep_for (std::chrono::milliseconds(10));
	...
	std::this_thread::sleep_for (std::chrono::milliseconds(10));
      ...
      // see: LATER at EOF: cvImage to ROS message, update rqt_image_view

      
    } // end: while(ros::ok()) ie exit ROS event loop.
...
// LATER at EOF: cvImage to ROS message, update rqt_image_view
//        
//
