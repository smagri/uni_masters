...
public:
  ...
  void seperateThread() {
	...
	if(!image.empty()){
	  ...
	  fprintf(stderr, "dbg: image.rows=%d, image.cols=%d\n",
	  ...
	  if (image.rows > 60 && image.cols > 60){
	    ...
	    std::cout << " pos [x,y]=[" 
		      ...
		      << " pos [x,y]=[" << (x/resolution_)+(image.rows/2)
		      << ","
		      << (y/resolution_)+(image.cols/2) << "]"
		      << std::endl;
	  }
...
int main(int argc, char **argv)
{
   ...
   * provided at the command line. For programmatic remappings you can
   * use a different version of init() which takes remappings
