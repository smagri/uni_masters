...
public:
    ...
    : nh_(nh), it_(nh)
  {
    ...
    // Getting command line parameters, with a default value also.
    //
    ...
    //
    // ogMap resolution, ie c-space ogMap resolution
  ...
  void  detectCellTypes(double closest_point, double angle,
    ...
    if (closest_point < range_min){
      ...
      fprintf(stderr, "dbg: detectCT: cp<range_min, cvPtr_->image.cols=%d\n",
	      cvPtr_->image.cols);
      // std::cerr << "dbg: detectCT: cvPtr_->image.cols=" 
      // 		<< cvPtr_->image.cols << std::endl;
	...
	cvPtr_->image.at<uchar>(py, px) == UK;
      ...
      std::cerr <<"dbg: detectCT: range_min<cp<range_max; cvPtr_->image.cols="
		<< cvPtr_->image.cols << std::endl;
