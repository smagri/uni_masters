...
public:
  ...
  void ogMap2global(int globalImageSize,
    ...
    /// environment)  coordinates.  The  global space  coordinates are
    /// then mapped to the "global" map(xg,yg) display window.
    ///
    ///
    ...
    //   (globalImageSize * resolution_)
      
    //   + y // y global space robot coord, no ogMap pixel offset applied
      
    //   + (py * resolution_); // y global space coord, ogMap offset applied
    // //yg = y + (py * resolution_);
    //similarly for py "global" map x coord
    //xg = (globalImageSize/2) + x + (px * resolution_);
    //xg = x + (px * resolution_);
    
    // // everything still an int:
    // yg = (globalImageSize/2) + py + (x/0.1);
    // xg = (globalImageSize/2) + px + (y/0.1);
	...
	if(!image.empty()){
	  ...
	  for (int py = 0; py<cvPtr_->image.rows; py++){
	    for (int px = 0; px<cvPtr_->image.cols; px++){
          ...
          // the  current  ogMap pixel  FT  values  to (xg,yg)  global
          // space, or robot environment???
          //
	  // for (int py=0; py<cvPtr_->image.rows; py++){
	  //   for (int px=0; px<cvPtr_->image.cols; px++){
	  //     if (cvPtr_->image.at<uchar>(py, px) == FT){
	  // 	ogMap2global(angleDeg, y, x, py, px, yg, xg);
	  ...
	  // double xDraw; // FT x coord to draw on "global" space
	  // double yDraw; // FT y coord to draw on "global" space
...
*/
	  ...
	  //   cv::Point2d (x1w, y1w);
	  //   cv::circle(globalImage,
	  // 	       cv::Point(x1w, y1w), 3, CV_RGB(255,0,0), -1);
	  //   // draw second waypoint
	  //   x2w = xWaypoint.at(i+1);
	  //   y2w = yWaypoint.at(i+1);
	  //   cv::Point2d (x2w, y2w);
	  //   cv::circle(globalImage,
	  // 	       cv::Point(x2w, y2w), 3, CV_RGB(255,0,0), -1);
	  //   // draw line between first and second waypoint
	  //   cv::line(globalImage, 
	  // 	     cv::Point(x1w, y1w), cv::Point(x2w, y2w),
	  // 	     CV_RGB(255,0,0), 1);
