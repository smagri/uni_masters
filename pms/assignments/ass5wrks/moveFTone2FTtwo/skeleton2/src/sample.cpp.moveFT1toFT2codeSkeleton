...
public:
  ...
  void detectFrontiers(cv::Mat &image, int py, int px,
	...
	if ((image.at<uchar>(y, x)!=O) && (image.at<uchar>(y, x)!=FS)){
...
//    int numPathPoses = poses.size();
  ...
  void seperateThread() {
	...
	if(!image.empty()){
	  ...
	  // Remove the current/first FT cell from the list of frontiers.
	  itFT = frontier.begin();
	  if (itFT != frontier.end()){
	    frontier.erase(itFT);
	  }

	  // Do not plot  the current/first FT cell when  we reach the
	  // next frontier.
	  // itGC = globalMapFTcoords.begin();
	  ...
	  // // grid unimpeded???
	  // //
	  // // LATER: do i need to avoid obsticals in going from FT to FT node???
	  ...
	  // location   the   pose/image/pose/laser  values   in   the
	  // respective  buffer/deques are  the same.
	  //
	  // With each invocation of the "while (ros::ok())" loop when
	  // moving to the new frontiers we can ignore the image/laser
	  // data and it's processing above,  as we are only concerned
	  // with  the  the  angle/orientation  and  position  we  are
	  // currently at relative to the last FT node.
	  //
	  // Similarly, when  determining and  processing FT  nodes on
	  // the  ogMap  at one  particular  FT,  where the  robot  is
	  // standing still, we can ignore the pose and orientation as
	  ...
	  //
	  // Step1:
	  // // Rotate the current robot pose to the angle the FT node is
	  // // from  it.  Move  angle to  the  left or  right about  the
	  // // z-axis.
	  // //
	  ...
	  // goalReq.x = goalReq.x + (itFT->second * resolution_);
	  // goalReq.y = goalReq.y + (itFT->first * resolution_);
