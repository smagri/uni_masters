...
*/
	  ...
	  // std::vector<double> xWaypoint = {0.0, 5.0, 10.0, 15.0, 0.0};
	  // std::vector<double> yWaypoint = {0.0, 5.0, 10.0, 15.0, 0.0};


	  // Waypoints are in metres as ROS uses SI units.  So need to
	  // be converted to pixels to display on the "view" window.
	  //
	  
	  // // The  waypoints are  displayhed in  black dots  with green
	  // // lines connecting them.
	  // fprintf(stderr, "waypoint vector size is %i\n", xWaypoint.size());
	  // for (int i=0; i<xWaypoint.size(); i++){
	  //   int x1 = (xWaypoint.at(i) / resolution_);
	  //   int y1 = (yWaypoint.at(i) / resolution_);
	  //   fprintf(stderr, "dbg:sThread: x1=%i, y1=%i\n", x1, y1);
	  //   cv::circle(rgbImage, cv::Point(x1, y1), 3, CV_RGB(0,0,0), -1);

	  //   int j=i+1;
	  //   int x2 =(xWaypoint.at(j) / resolution_);
	  //   int y2 = (yWaypoint.at(j) / resolution_);
	  //   fprintf(stderr, "dbg:sThread: x2=%i, y2=%i\n", x2, y2);
	  //   cv::circle(rgbImage, cv::Point(x2, y2), 3, CV_RGB(0,0,0), -1);
	    
	  //   cv::line(rgbImage, 
	  // 	     cv::Point(x1, y1), cv::Point(x2, y2),
          //            CV_RGB(0,255,0), 1);

	  //   fprintf(stderr, "dbg:sThread: i=%i\n", i);
	  // }
	
	  // Template for drawing a path between waypoints, project
	  // requirement, using PIXELS.
	  // cv::circle(rgbImage,
	  ...
	  //
	  // // Template for drawing a path between waypoints, project
	  // // requirement, using METRES.
	  // cv::circle(rgbImage, cv::Point(x, y), 2, CV_RGB(0,0,255), -1);
	  // cv::circle(rgbImage,
	  // 	     cv::Point( (x + (image.rows * resolution_)),
	  // 			(y + (image.rows * resolution_)) ),
	  // 	     4, CV_RGB(0,0,255), -1);
	  // cv::line(rgbImage, 
	  // 	   cv::Point(x, y),
	  // 	   cv::Point( (x + (image.rows * resolution_)),
	  ...
	  // int x2, y2;
 	  // x2 =( (x+(image.rows*resolution_)) + (image.rows/2) );
	  // y2 =( (y+(image.rows*resolution_)) + (image.rows/2) );
	  // cv::circle(rgbImage, cv::Point( x2, y2), 4, CV_RGB(0,0,255), -1);
	  
	  // cv::line(rgbImage, cv::Point(x, y), cv::Point(x2, y2),
	  // 	   CV_RGB(255,0,0), 1);
