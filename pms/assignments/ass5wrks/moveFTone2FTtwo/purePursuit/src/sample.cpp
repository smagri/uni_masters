...
//    double msg->angle_min = (angleDeg*(pi/180)); // start angle in radians
  ...
  //   // should this be BFS?? to next FT node
    ...
    std::vector<double> waypoints2FT; // LATER: make as a structure
    std::vector<std::vector<double> > path2FT; // LATER: make as a linked list
  ...
  void seperateThread() {
    ...
    //
    // Execute ros::ok() loop every 10 seconde.  That is 10 seconds
    // since the last ros Rate::sleep()
    ros::Rate loop_rate(10);  // sleep at rate of 10Hz
    // OR ros::Duration(10).sleep(); //
    
    
    while (ros::ok()) {

      // When ros::ok() returns false eg:
      //   ctri-c, SIGINT,
      //   ros::shutdown(),
      //   all ros nodes have been distroyed
      // all ros calls will fail.
	...
	if(!image.empty()){
	  ...
	  // Move to next unvisited FT cell *************************
	  //
	  // Moving the robot to  the next unvisited frontier requires
	  // moving the robot, along an obstacle free path.  This path
	  // is returned by  a call to the  request_goal service.
	  //
	  // The robot  attempts to move  along this path  however its
	  // electrical and  mechanical nature  cause it to  stray off
	  // course at  times, thus  not tracking the  path perfectly.
	  // To allow corrections to be  made to the actual robot path
	  // while attempting to move along  the desired or ideal path
	  // I  will use  a Pure  Pursuit control  algorithim.  For  a
	  // decription  of this  method see  the following  document,
	  // which  will  be  refered  to as  [Coulter]  from  now  in
	  // https://www.ri.cmu.edu/pub_files/pub3/coulter_r_craig_1992_1/
	  // \coulter_r_craig_1992_1.pdf
	  //
	  // Using this  algorithim one  must determine  the curvature
	  // that will move a robot  from its current position to some
	  // goal position.  One needs to  choose a goal position that
	  // is some distance ahead of the robot on the path, known as
	  // the lookahead distance, see figure 1 [Coulter].  Changing
	  // the lookahead  distance can  have significant  effects on
	  // the performance of the algorithm[Lundgren]:
	  // http://www8.cs.umu.se/kurser/TDBD17/VT06/utdelat/Assignment \
	  // %20Papers/Path%20Tracking%20for%20a%20Miniature%20Robot.pdf
	  // 
	  // Here I  am assuming  a curvy path  with the  robot moving
	  // slowly.  So  I will intially  be using a  small lookahead
	  // distance.  With  a view to reviewing  this once emperical
	  // results are obtainied.
	  //
	  // Broadly  speaking,  I  will implement  the  Pure  Pursuit
	  // algorithim  by  first  deteriming the  curvature  of  the
	  // desired path at its lookahead distance.  When the desired
	  // path it  at its lookahead  distance the curvature  of the
	  // actual  path will  be  calulated.  The  curvature of  the
	  // desired and actual path will  then be compared allowing a
	  // decision to be made as to  how to correct the actual path
	  // of  the robot  to  get it  as close  as  possible to  the
	  // desired path of the robot???
	  
	  // So firstly  we need  to move the  robot along  the x-axis
	  // till we reach the lookahead point on the path.
	  //
	  // Thus foreach  movement time  step of the  robot (x,y,yaw)
	  // values need  to be  buffered till  we reach  the lookehad
	  // point, then calculate the desired and actual curvature.
	  
	  // Path  buffer  length  probably   needs  to  be  deterined
	  // emperically so that path points are not lost.
	  
	  // Current desired  path vector, returned  by a call  to the
	  // "reques_goal" service.
	  //
	  // ***This will  be a  _series_ of x,y,yaw  values OR  a single
	  // x,y,yaw set****
	  
	  // Curent actual path vector, a series of waypoints to next FT
	  // OR is one waypoint returned at a time thus would be insinc with
	  // odomPath buffer
	  //
	  // Original x,y,yaw is where the robot starts from
	  path2FT[x,y,yaw]; 

	  // probably odomPath should be a linked list of structures
	  //
	  // odom buffer should be of size  path2FT.size()
	  //
	  // how do i fill the odomPath buffer???, multiple
	  // invoications of sThread???
	  odomPath[x,y,yaw];
	  

	  // Curvature is calculated is phi = 2x / lookaheadDistance^2.

	  // The only parameter of the  pure pursuit algorithim is the
	  // lookahead  distance.   lookahead  distance chosen  to  be
	  // 0.2m, the resolution of the ogMap Current odometry vector
	  // is probably a good palce to start.
	  double lookaheadDistance = 0.2m; // l = lookahed

	  // Move the robot in global  space along the x-axis till the
	  // desired path point one lookahead distance from the origin
	  // is attained.
	  //
	  // Note that the  robot moves a certain  small distance with
	  // one movement command,  so as not to bump  into things, so
	  // the movement commands need to sent in a loop.
	  //
	  // One robot time step.
	  // Default constructor sets all commands to 0.
	  geometry_msgs::Twist robot_base_cmd;
	  double pathDistanceThreshold = 0.2; // LATER: maybe 0.3, test

	  // We must loop here till all the waypoints in the path have
	  // been trasversed and we are at the next FT node.
	  //
	  ros::Rate rate(10); // loop while(ros::ok) 10 times a second
	  while(ros::ok(){

	      double yp; // y of path waypoint
	      double xp; // x of path waypoint
	      double yawp; // yaw of path waypoint

	      // current distance on path to lookahead point, or goal point
	      double cureDistanceOnPath;

	      // previous distance on path to lookahead point, or goal point
	      double prevDistanceOnPath;

	      cureDistanceOnPath = 0;
	      prevDistanceOnPath = 0;

	      
	      while (!pathFT.size()){

			
		// Get next path waypoint.
		for (int=i; i<waypoints2FT.size(); i++){
		  // these variables should be in a struct which would
		  // be put in a linked list

		  // make path2F a deq, so these lines need fixing
		  waypoints2FT(i) = path2FT.pathDeq.front();

		  // Get  current   x,y  pairs  on  desired   path  ie
		  // currentPathX, currentPathY
		  yCurPath = pathFT.waypoints2FT.y;
		  xCurPath = pathFT.waypoints2FT.x;
		  yawCurPath =pathFT.waypoints2FT.yaw;
		  pathFT.waypoints2FT.pop_front();
		}


		curDistanceOnPath =
		  prevDistanceOnPath + sqrt(pow(yCurPath,2) + pow(xCurPath,2));

		if( (currentDistaceOnPath
		     >= (lookaheadDistance-pathDistanceThreshold))
		    && (currentDistaceOnPath
			<=(lookaheadDistanced+pathDistanceThreshold)) ){
		  
		  // Stop moving robot as we are at the lookahead point(x,y)
		  // ie current (currentPathX,  currentPathY).  This is also
		  // known as the goal point.
		  robot_base_cmd.linear.x = 0;
		  robot_base_cmd.angular.z = 0;
		  cmd_vel_pub_.publish(robot_base_cmd);
		}
		...
		// Note   that   moving   the   robot   initiates   an
		// odomCallback().   And   it  will  be   queued  till
		// spinOnce() executes.
				
		// linear velocity(metres/(time step)) == linear distance
		robot_base_cmd.linear.x = 0.2; // move robot forward
		
		// angular velocity(radians/(time step)) == anglar
		// distance, yaw
		robot_base_cmd.angular.z = 0;
		
		spinOnce(); // allow odomCallback() to execute
				
		// get current robot pose, ie after it was moved forward above
		geometry_msgs::Pose pose=msg->pose.pose;
		buffer.buffer_mutex_.lock();
		buffer.poseDeq.push_back(pose);
		buffer.timeStampDeq.push_back(msg->header.stamp);
		...
		// Transform goal point which is  in global space to robot
		// coordinates.
		xGoalPoint = x;
		yGoalPoint = y;
		yawGoalPoint = yaw;
		
		// LATER: is  may be  oriented wrong  ie z-axis  should be
		// pointing up as that is what ROS uses.
		double xGoalPointInRobotCoords
		  = ( ((xGoalPoint-x)*cos(yaw))
		      + ((yGoalPoint-y)*sin(yaw)) );

		double xGoalPointInRobotCoords
		  = ( (-1*((xGoalPoint-x)*sin(yaw)))
		      + ((yGoalPoint-y)*cos(yaw)) );

		// Comput the desired curvature of the robot:
		//
		// Curvature is phi = 2x / lookaheadDistance^2,
		//
		// where  x is  the ofset  along the  x-axis from  the
		// robot pose to the lookahead point or goal point.

		desiredCurvature = 2 * x / pow(lookaheadDistance,2);
		actualCurvature = 2 * x / pow (actualLookahedDistance,2);

		if (actualCurvature < desiredCurvature){
		  // move the robot aroud the z-axis clockwise until
		  //the difference in curvature is at a certain
		  //threshold
		  //robot_base_cmd.angular.z = -ve;
		}
		else{
		  // actualCurvature > desiredCurvature)
		  // move the robot around the z-axis anti-clockwise until
		  // the difference in curvature is at a certain
		  // threshold
		  //robot_base_cmd.angular.z = +ve;
		}
	      ...
	      // The callbacks waiting will be called
	      //
	      // This  allows callbacks  to still  execute during  the
	      // slow process of moving it  along a path.  Also thread
	      // should be  allowed to  hog the cpu,  or execute  in a
	      // tight loop.
	      ros::spinOnce();

	      // sleep till ros::Rate object called rate expires, this
	      // includes the time taken to execute this loop
	      ros::rate.sleep();
	      
	    } // end: while(ros::ok)

	      
	    // Impement this as a heuristic of the Pure Pursuit algorithim.
	    //
	    // // Compute  the  angle   difference  between  the  robot's
	    // // heading and the look-ahead point direction.
	    // double thetaLookaheadPoint = 
	    // atan2(yCurrentPath / xCurrentPath);
		
	    // double deltaThetaThreshold;
	    // double detaTheta = (thataLookaheadPoint - yaw);
		
	    // if (deltaTheta is +ve and > deltaThetaThreshold){
	    //   // rotate right
	    //   robot_base_cmd.angular.z = -ve
	    //     }
	    ...
	    // }
	    //
	    // remember for ROS moving robot forward is +ve and
	    // backward is -ve
	  ...
	  // Move to next unvisited FT cell, some ideals**************
	  //
	  //
...
*/	  
	...
	// This delay slows the loop down for the sake of readability,
	// provided in  example code.  However, it  should be replaced
	// by loop_rate.sleep() so it is properly controlled by ros.
	std::this_thread::sleep_for (std::chrono::milliseconds(10));
	loop_rate.sleep();
