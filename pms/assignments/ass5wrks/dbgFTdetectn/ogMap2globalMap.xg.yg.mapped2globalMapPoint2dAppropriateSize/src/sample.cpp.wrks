...
public:
  ...
  void ogMap2globalMap(double globalImageSize, cv::Mat image, 
    ...
    xg = ((globalImageSize / 2)
    	  + x
    	  + (pxOffset * resolution_));
...
//    int numPathPoses = poses.size();
  ...
  void seperateThread() {
	...
	if(!image.empty()){
	  ...
	  //
	  // x1 = globalMapFTcoordsXG.front();
	  // y1 = globalMapFTcoordsYG.front();
	  // cv::Point2d p1(x1, y1);
	  // cv::circle(globalImage, cv::Point2d(p1), 4, CV_RGB(0,0,255), -1);
	  // globalMapFTcoordsXG.pop_back();
	  // globalMapFTcoordsYG.pop_back();
	  //
	  // x2 = globalMapFTcoordsXG.front();
	  // y2 = globalMapFTcoordsYG.front();
	  // cv::Point2d p2(x2, y2);
