...
public:
    ...
    : nh_(nh), it_(nh)
  {
    ...
    void ogMap2globalMap(double globalImageSize, 
			 double frontierAngle, double y, double x,
  ...
  void detectFrontiers(cv::Mat &image, int py, int px,
    ...
    if (image.at<uchar>(py, px) = FS){
	  ...
	  // wrks: frontier.insert(std::make_pair(std::make_pair(py,px), 90.0));
  ...
  void ogMap2globalMap(double globalImageSize,
    ...
    /// frontierAangle, adjacent angle in degrees, input
    ///
    /// ROS uses SI units so the global coordinates are in metres.
    ///

    if ( (frontierAngle>=0) && (frontierAngle<=90) ){
      px = px;
      py = py;
    }
    else if ( (frontierAngle>90) && (frontierAngle<=180) ){
      fprintf(stderr, "dbg: ogMap2globalMap: "
              "90>frontierAngle<=180, before px=-px, px=%d\n", px);
      px = -px; // (px * -1) ???
      py = py;
      fprintf(stderr, "dbg: ogMap2globalMap: "
              "90>frontierAngle<=180, after px=-px, px=%d\n", px);

    }
    else if ( (frontierAngle>180) && (frontierAngle<=270) ){
      px = -px;
      py = -py;
    }
    else if ( (frontierAngle>270) && (frontierAngle<=0) ){
      px = px;
      py = -py;
    }
    else{
...
//    int numPathPoses = poses.size();
  ...
  void seperateThread() {
	...
	imageBuffer.timeStampDeq.pop_front();
      ...
      std::vector<std::pair<double, double> > globalMapCoords; // yg,xg	  
	...
	if(!image.empty()){
	  ...
	  for (itFT=frontier.begin(); itFT!=frontier.end(); itFT++){
	    ...
	    double angleFT = itFT->second; // get Value
	    //std::cout << "dbg: pyFT=" <<pyFT<< " pxFT=" <<pxFT<< std::endl;
	    ogMap2globalMap(globalImageSize, angleFT, y, x, pyFT, pxFT, yg, xg);
	    
	    globalMapCoords.push_back(std::make_pair(yg, xg));
	  ...
	  // for (itGC=globalMapCoords.begin(); itGC!=globalMapCoords.end();
	  //      itGC++){
...
*/
	  ...
	  //
	  resolution_ = 1; // temporarily for testing purposes
	  // angle of UK  adjacent the FS = 180, blue
	  for (int i=0; i<(xWaypoint.size()-1); i++){
	    // draw fist waypoint
	    ogMap2globalMap(globalImageSize, 90,  y, x,
			    yWaypoint.at(i), xWaypoint.at(i), yg, xg);
	    cv::Point2d p1(yg, xg);
	    cv::circle(globalImage, cv::Point2d(p1), 8, CV_RGB(255,0,0), -1);
	    
	    // draw second waypoint
	    ogMap2globalMap(globalImageSize, 90, y, x,
			    yWaypoint.at(i+1), xWaypoint.at(i+1), yg, xg);
	    cv::Point2d p2(yg, xg);
	    cv::circle(globalImage, cv::Point2d(p2), 8, CV_RGB(255,0,0), -1);
