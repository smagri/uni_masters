...
return;
  ...
  void seperateThread() {
	...
	if(!image.empty()){
...
#endif // end: DEBUG_DRAW_ON_IMAGE


#ifdef DEBUG_DRAW_BLACK_IMAGE
	  ...
	  // Thus as the  ogMap image varies, see  'Image View' window
	  // subscribed to  /map_image/full topic  via rqt_image_view,
	  // this  code causes  each image  to  turn a  full black  in
	  // colour.   The  ogMap  image   is  generated  by  the  ros
	  // map_to_image_node.
	  //
...
#endif // end: DEBUG_DRAW_BLACK_IMAGE
	  ...
	  // map_to_image_node???
	  // Should map_to_mage_node do somthing with this message???
	  // Should map_to_image_node subscribe to,
	  // image_pub_ = 
	  //  it_.advertise<sensor_msgs::ImagePtr>("map_image/full", 1);
	  //
	  sensor_msgs::ImagePtr msg
