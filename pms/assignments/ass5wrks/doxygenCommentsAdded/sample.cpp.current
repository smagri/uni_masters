...
//  bool requestGoal(RequestGoal::Request  &req,
//		   RequestGoal::Response &res)
  {
...
//	     req.x, req.y, req.angle, req.scanTime);
  ...
  void seperateThread() {
	...
	if(!image.empty()){
	  ...
	  // Clearest  method,  that  is  the one  that  causes  least
	  // confusion,  is  used here  to  indicate  which class  the
	  // objects belong to. Using:
	  //
	  // Request rgreq;
	  // Response rgres
	  //
	  // will work as this class, GazeboRetrieve, derives from the
	  // class a3_help::RequestGoal, hence all the member data and
	  // methods   of   a3_help::RequestGoal  are   available   to
	  // GazeboRetrieve.
	  //
	  a3_help::RequestGoal::Request sampleReq;
	  a3_help::RequestGoal::Response sampleRes;
	  ...
	  double angleDegrees = 0.0;
	  sampleReq.angle = (angleDegrees * (pi/180)); // angle in radians
	  //
	  sampleReq.scanTime = 10; // time between scans in seconds
	  //
	  sampleRes.ack = false;
	  ...
	  bool success = client_.call(sampleReq, sampleRes);
