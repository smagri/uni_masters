...
vector<double> RangerFusion::getFusedRangeData(){
  ...
  //
  //rangers_.at(numValues2fuseIndex)->getResolution();
  // fovMax = 180 degrees???
  double angle[numValues2fuse];
//  double sensorRes = rangers_.at(numValues2fuseIndex)->getConfigAngularRes();
  // double sensorRes = rangers_.at(0)->getSensorRes();
  // double sensorRes = 30;
  // for (int i=0; i<numValues2fuse; i++){
  //   angle[i] = sensorRes * i;
  //   fprintf(stderr,"angle[%d]=%f\n", i, angle[i]);
  // }
  vector<double> fusionAngle;
  double sensorRes = 30; // LATER: NEED TO MAKE THIS GENERIC
  ...
  for (int i=0; i<rangers_.size(); i++){
	...
	fusionAngleFlag[i].push_back(1); // sample included fusion
      }
      else{
  ...
  vector<double> fusedDataVec; // vector of fused data
