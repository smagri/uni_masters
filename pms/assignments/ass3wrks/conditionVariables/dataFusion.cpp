...
void DataFusion::processBuffers(){
  ...
  while(true){
	...
	if (!dataBuff[rangerNum].prevDatasetPointExists){
	  ...
	  x2 = dataBuff[rangerNum].buffQue_.front();
	  dataBuff[rangerNum].buffQue_.pop();
	  
	  y1 = dataBuff[rangerNum].yPrev;
	  y2 = dataBuff[rangerNum].buffQueTimestamp_.front();
	  dataBuff[rangerNum].buffQueTimestamp_.pop();
	  ...
	  linearExtrapolation(x1, y1, x2, y2, x, y);
	...
	// mutex: dataBuff[rangerNum].buffMutex_.unlock();
	// condition variable: 
	dataBuff[rangerNum].producer_.notify_all();//wake producer/fillBuffer()
...
void DataFusion::linearExtrapolation(double x1, double y1,
  ...
  fprintf(stderr, "dbg: linearExtrapolation: (x1, y1)= (%f, %f);\n"
	  "dbg: linearExtrapolation: (x2, y2) = (%f, %f);\n"
	  "dbg: linearExtrapolation: (x, y) = (%f, %f))\n",
	  x1, y1, x2, y2, x, y);
