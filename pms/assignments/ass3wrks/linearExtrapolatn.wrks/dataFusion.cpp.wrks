...
void DataFusion::processBuffers(){
  ...
  while(true){
	...
	if (!dataBuff[rangerNum].prevDatasetPointExists){
	  ...
	  fprintf(stderr, "dbg: processBuffers: No previous point in dataset.\n"
	...
	fprintf(stderr, "dbg: processBuffers: "
		"( (x1,y1)=(%f,%f), (x2,y2)=(%f,%f), "
		"(dataBuff[rangerNum].xPrev,dataBuff[rangerNum].yPrev)=(%f,%f),"
		"(x,y)=(%f,%f))\n", x1, y1, x2, y2,
		dataBuff[rangerNum].xPrev, dataBuff[rangerNum].yPrev,
		x, y);
...
void DataFusion::linearExtrapolation(double x1, double y1,
  ...
  fprintf(stderr, "linearExtrapolation: ((x1, y1)=(%f, %f), (x2, y2)=(%f, %f),"
	  " (x, y)=(%f, %f))\n", x1, y1, x2, y2, x, y);
