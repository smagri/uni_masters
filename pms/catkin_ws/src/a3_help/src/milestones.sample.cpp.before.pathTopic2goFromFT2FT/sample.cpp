...
class GazeboRetrieve : public a3_help::RequestGoal {
  ...
  // The ogMap  is a  greyscale image,  hence the  following intensity
  // values for an image cell are known:
  uchar FS = 255; // Free Space/Cell
  uchar O = 0;    // Occupied Space/Cell
  uchar UK = (!FS && !O); // Uknown Cell, vaule ranges from 1 to 254.

  // FT  is just  an arbitary  symbolic number  assigned to  an openCV
  // image  cell   for  the  purposes  of   the  algorithms,  frontier
  // exploration, in this program.  If there  is a UK cell adjacent to
  // a FS cell that  FS cell is labeled a FT  cell.  And remember when
  // the robot environment is fully mapped  there are no more UK cells
  // so you can set  FT to any value that is an uchar  but is not a FS
  // or O cell,  as FS and 0 cells are  valid fully mapped environment
  // cell values.
...
public:
    ...
    : nh_(nh), it_(nh)
  {
    ...
    void ogMap2globalMap(double globalImageSize, 
    		      double angleDeg, double y, double x,
                      int py, int px, double &yg, double &xg);
    // // for int:
    // void ogMap2globalMap(int globalImageSize, 
  ...
  void ogMap2globalMap(double globalImageSize,
    ...
    if ( (angleDeg>=0) && (angleDeg<=90) ){
      ...
      fprintf(stderr, "dbg: ogMap2globalMap: "
      ...
      fprintf(stderr, "dbg: ogMap2globalMap: non valid angle\n");
    }
  ...
  void  rangeVectorNextPxPy(int angleDeg, int &px, int &py){
    ...
    std::cerr << "dbg: rvNextPxPy: angleDeg = " << angleDeg << std::endl;
    ...
    if ( (angleDeg >= -10) && (angleDeg <= 10) ){
      ...
      std::cerr << "dbg: rvNextPxPy: angleDeg out of range, and is="
		<< angleDeg << std::endl;
    }
...
//    int numPathPoses = poses.size();
  ...
  void seperateThread() {
	...
	imageBuffer.timeStampDeq.pop_front();
      ...
      std::vector<double> globalMapCoordsXG; // mapped global x coord stored
      std::vector<double> globalMapCoordsYG; // mapped global y coord stored
	...
	if(!image.empty()){
	  ...
	  for (int py =0; py<image.rows; py++){
		...
		std::cerr << "dbg: sThread: FT found" << std::endl;
	  	...
	  	ogMap2globalMap(globalImageSize,
				angleDeg, y, x, py, px, yg, xg);
	  	globalMapCoordsYG.push_back(yg);
	  	globalMapCoordsXG.push_back(xg);
	  ...
	  for (int i=0; i<globalMapCoordsXG.size(); i++){
	    xDraw = globalMapCoordsXG[i];
	    yDraw = globalMapCoordsYG[i];
	  ...
	  //
	  // x1 = globalMapCoordsXG.front();
	  // y1 = globalMapCoordsYG.front();
	  // cv::Point2d p1(x1, y1);
	  // cv::circle(globalImage, cv::Point2d(p1), 4, CV_RGB(0,0,255), -1);
	  // globalMapCoordsXG.pop_back();
	  // globalMapCoordsYG.pop_back();
	  //
	  // x2 = globalMapCoordsXG.front();
	  // y2 = globalMapCoordsYG.front();
	  // cv::Point2d p2(x2, y2);
